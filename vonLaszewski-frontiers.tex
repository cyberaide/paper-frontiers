...
\subsection{Input and Output Hyperparameters}

For the rest of the experiments we have chosen different numbers of
input and output variables for our deep learning algorithm.
...
\TODO{Verify if the ranges seek backwards or forwards when sampling}
  ...
  In the subsequent benchmark results, groupings of data were used as
  hyperparameters and leveraged as training data to optimize their
  associated accuracy values.

  The data was categorized using the following rules
  ...
  {\em 1Y 2wk+26AVG} is the resulting dataset containing a grouped set of values
  representing 2-week periods, seeking 1 year back from the most recent observation
  and walking backward until 26 groups have been included.  As another
  example, the value of {\em 1 Year Back} is a single 2 week occurrence one 
  year ago from the rolling training date.
...
rank, nnse, epoch, name
17, 0.5818, 2, 2 weeks Now
40, 0.4473, 2, 2 weeks Now
56, 0.4098, 2, 2 weeks Now
65, 0.3800, 2, 2 weeks Now
109, 0.2969,2, 2 weeks Now

[5:30 PM]J.P.: VALIDATION
rank, nnse, epoch, name
