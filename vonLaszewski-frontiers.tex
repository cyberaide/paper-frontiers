...
\item {\bf Teamwork.} Students benefit from focusing on the success
  ...
  \item {\bf Software ecosystem.} Students are often only using a class provided
    limited custom defined ennvironment prepared explicitly for a class that makes
  ...
  \item {\bf IO Data management.} One other important lesson is the
    ...
    DL algorithms require a large number of fast IO interactions. Havving access to sufficiennt space to store potentially larger datasets is beneficial. Also the time id need to send data from the external storage to the GPU shoudl be small as to assure that the GPUs have sufficient data to perform well without bottelneck.
...
around it. For this to work, the course can be focusing on deep
...
In contrast to other courses that may only focus on DL technieques
...
\item{\bf Project Presentations:} The last part of the class is
  ...
  Ideally each team will submit its result to MLCommons. A good start here is the Science Working group as it provides rolling submissions and its focus is accuraccy and not speed which is often a topic of interest in academia.
...
generation a special batch script that internally uses Papermill \citep{www-papermill} to
...
that the performance was more then twice as fast on the personal computer while using an RTX3090
...
the ability to call a Stowatch.event.
...
The analysis must include a an easy-to-understand specification
...
After the code augmentations, we were able to achieve a projected runtime on
...
RTX3090 GPU performed significantly faster than the HPC compute node
...
application’s accuracy, this next section reports the accuracy of
