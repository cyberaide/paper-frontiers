...
\TODO{Done - Butler}{what is number in brackets?  Butler-done.}
...
\TODO{Done - Butler}{The TFT model is not sufficiently explained. Butler- done.}
...
70 epoch runs. In the future, we intend to test epoch runs greater than
70 in order to identify overfitting boundaries.
...
\TODO{ALL}{Figure 6 Loss, this image seems wrong. It is weird for train to have a higher loss than validation for most of the graph. Are the values inverted?}
...
Thus, applying these rules, a range of values is notated by first
...
date run on the dataset and including 26 observations prior this point.
...
\TODO{Butler}{add here the explicit wording just for this experiment parameters}

Although we see on average better accuracy results with 30 epochs, the best accuracy values we still have seen with 70 epochs. This indicates that we have a higher variation of the results in the 70 epoch case. However as we are competing for the best value and not the average value this variation has an advantage and we do not as easily get stuck in a local minima. 
...
An important observation is for the training phase (colored in green) the energy values of the GPU are significantly higher then in the rest of the application indicating a much higher utilization of the GPU during that phase. 
...
In Figure~\ref{fig:energy-graphs-compare}B, we also depicted the Energy trace per epoch. However in contrast to Figure~\ref{fig:energy-graphs-compare} A that were created using the data on an NFS storage device, we included here the values obtained for data hosted on a localscratch device using NVMe storage. 
Here we see a similar average energy use per epoch. 

\TODO{JP and Knuuti}{Figure~\ref{fig:energy-graphs-compare} B should add four bars, a100 NFS and localscratch, v100 NFS and localscratch}

This comparison of using localscratch vs. NFS mounted data shows that 

\TODO{JP and KNuuti}{TBD}


Hence it offers a clear insight how a filesystem can negatively impact the energy as the runtime is enhanced and the GPU time to cool down or adjust for reducing the energy is to small to have an impact.
