...
was conducted across several educational efforts and has identified
...
time-related benchmarks. The activity was only possible by addressing
a simplification of the benchmark pipeline while developing and using
software to automatically generate jobs from the permutation of
hyperparameters while using a template job management framework
...
\TODO{Robert}{Help Thomas and Jake, spellcheck paper. provide TODOs if you see something major.}
...
The work reported here has been conducted as part of the MLCommons Science Group goals.
...
learning a small number of edit commands.
  ...
  leads to more sustainable and easier to engage code. Often we found that code
  developed as Jupyter notebook frequently resulted in missing
  documentation although notebooks have an integrated markdown engine that can provide sophisticated documentation built in. This relates to a general problem at the
...
\begin{comment}
  ...
  However, we experienced that centers that supports container images to be run
  by its users did not supply the the software necessary to create the
  images, making the viability of using container technology limited.

\end{comment}

\todo{GVL}{The prior phrase said "campus HPC Centers".  Is campus significant, or can we simplify to HPC Centers in a more general sense?}
...
Software benchmarks not only needs the newest software
...
The MLCommons benchmarks provides a valuable starting point for
...
\TODO{GVL}{The below has a quote, but lacks an attached citation.  Should the citation to las-22-mlcommons-science move up before the table, or did this come from somewhere else?}
...
\begin{figure}[htb]
    \centering\includegraphics[width=0.5\columnwidth]{images/TFT.pdf}
...
\begin{itemize}
  \item {\bf Static Known Inputs (5, number of inputs):} 4 space-filling curve labels of fault grouping, linear label of pixel
  \item {\bf Targets (4):} $m_{bin} (F:\Delta t,t)$ for $\Delta t = 2, 14, 26, 52$ weeks. Calculated for $t-52$ to $t$ for encoder and $t$ to $t+52$ weeks for decoder in $2$ week intervals. 104 predictions per sequence.
  \item {\bf Dynamic Known Inputs (13):} $P_l(\cos_{Full})$ for $l=0$ to $4$ $\cos_{period}(t), \sin_{period}(t)$  for $period = 8, 16, 32, 64$
  \item {\bf Dynamic Unknown Inputs (9):} Energy-averaged Depth, Multiplicity, Multiplicity  $m>3.29$ events $m_{bin}$ $(B:\Delta t,t)$ for $\Delta$ $t = 2, 4, 8, 14, 26, 52$ weeks
\end{itemize}

\TODO{Butler}{what is number in brackets?  Butler-done.}
...
For more detail on the current TFT model, review~\citep{fox2022-jm}. For more general details on TFT models, review~\citep{TFT-21}.

\TODO{Butler}{The TFT model is not sufficiently explained. Butler- done.}


\subsubsection{Insights into Development of the Code}

\TODO{GVL}{Confirm that the DL method is called tevelop; if so, is there a citation for this method?}
...
The benchmarks we present in the next sections have been run on a number of compute resources. This includes not only an HPC at UVA but also a desktop and a laptop to represent different classes of compute resources that students have access to but are not part of an HPC center. We used the following resources:
...
2 epoch case for modelling prediction and the rest of the application . The bars in the histogram are devided while the top part indicates the model prediction and the bottom part is the rest of the runtime of the application. show the preparation as bottom part of the bar and the prediction as the top part of the bar
...
runtime.  The total runtime for the initial configurations using various configurations with different GPUs and filesytsems where available on the HPC is shown in
...
was initially located on an NSF storage system.  As we see the desktop with an RTX3090 is significantly faster than the HPC compute node
...
Base on these finding we motivated a change in the architecture of the HPC server nodes to add a file system with NVMe storage on it. However as our measurements show the desktop significantly outperformed the HPC system even after this upgrade.

However, we were only running the desktop up to 33 epochs and the dotted line includes our prediction of the runtime based on our previous values. The reason for this is that during our experiments we ran out of memory on the desktop to produce the visualization which are part of the overall program. Furthermore the desktop was also used in multi-user mode as it presented the main work computer of the researcher and other activities needed to be done on this machine in parallel. Nevertheless, it is quite impressive, what a difference a particular processor, memory and file system hardware has on the overall performance. 
 One must assure that the the data feed to the GPU can keep up with the GPUs performance.
...
\begin{figure}[p]
  ...
  \centering\includegraphics[width=0.9\columnwidth]{images/Benchmark_comp_resource_new.pdf}
...
\TODO{All}{figure \ref{fig:performance-projection} B is now correct and needs to be described}

\TODO{GVL}{add timer portions and what they are}
...
\begin{table}[htb]
    ...
    {\footnotesize              
          ...
          \begin{tabular}{|lrrrrr|}
            ...
            Initialization    &    0.8  &    3.6  &    8.1  &     5.6 &     5.3 \\
...
\TODO{Butler}{What are the different portions, describe, I think we need to move thie somewhere earlier and refer to it ....}

\begin{itemize}
    \item {\bf Total} -- This shows the total time the model took to run.
    \item {\bf Initialize - Table \ref{tab:2-epoch-case}} -- Time taken to initialize all static values and flags.
    \item {\bf Initialize - Figure \ref{fig:energy-graphs}} -- Time taken to initialize all data.
    \item {\bf Sampling locations} -- Time taken to sample time series data into 2 week batches.
    \item {\bf Training / Model Fit} -- Time taken to train model and determine and save best fit from all epochs.
    \item {\bf Bestfit or Bestfit Prediction} -- Time taken to take best fit model an calculates predictions. Then calculates Mean Squared Error (MSE) and NNSE on predictions.
    \item {\bf Visualize} -- Time taken to gather data collected during model runtime and move to a permanent directory and process plots and graphs from data.
    \item {\bf Final Plots} -- Time taken to clean up data and close all unneeded processes.
\end{itemize}

\TODO{ALL}{In energy we have different/additional values, somehow we need to describe them but have only one list. This needs to be done carefully as we do not want to falsify results or introduce confusion.}
...
hyperparameters and leveraged as training data to optimize their 
associated accuracy values. The data is categorized using the following rules
...
example, a value of {\em 1Y 2wk+26AVG} is one year from the most recent 
date run on the dataset and including 26 observations from this point.
...
values, while in Figure \ref{fig:NNSE-comparison-a100} we created a
...
Although we are mostly interested in identifying the best accuracy value, it is also advantageous to reflect that data places the 70 epoch case above the other values, generally leading to better results (see Figure \ref{fig:NNSE-comparison-a100}).
...
\TODO{DONE, GVL}{Integrate in text could we sort Figure \ref{fig:performance-projection} values on x axis by best occurrence of the  value for that hyper parameter.
\\~\\
e.g. this requires to find the bets value for the time, for 2, 30, 70 epochs, and than order that and then use that order in the display. This way the best value is an increasing function over all time periods. One could also do this by hand and just reorganize the order on the x axis -- the easiest way to do this is to just look at the overall to table and go through the list and see in which order the time comes write it down and than just use that order. -- or loop and identify all values for a time parameter and than us max. We want order based on validation
\\~\\
fields = set ( get column with prediction parameters )
\\~\\
for field in fields: 
~~ get all accuracy for this field from 2,30,70 and find the maximum store in order = field, max value. (there is then only one value for field
\\~\\
take order and sort by max value, print and get the order from now sorted fields
use sorted fields for order in prediction parameters}

\begin{figure}[p]

  \begin{center}
       \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=1.0\linewidth]{images/frequency_nnse_histogram_stacked_training.pdf}
        {\bf (A)} Histogram of the accuracy values for every epoch over all experiments for training.
     \end{minipage}
  \ \
     \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=1.0\linewidth]{images/frequency_nnse_histogram_stacked_validation.pdf}
        {\bf (B)} Histogram of the accuracy values for every epoch over all experiments for validation.
     \end{minipage}
  \ \
     \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=1.0\linewidth]{images/A100-NNSE-all-epochs-training-min-max.pdf}
        {\bf (C)} Accuracy values over all experiments for training sorted by best validation values.
     \end{minipage}
  \ \
     \begin{minipage}[t]{0.49\textwidth}
        \includegraphics[width=1.0\linewidth]{images/A100-NNSE-all-epochs-validation-min-max.pdf}
...
The best value for the prediction parameter, {\em Now 2wk+26AVG}, is 0.6930 for training and 0.5582 for validation (see Table~\ref{tab:ranking-accuracy}).
...
However, it is importnatto also consider other applications when measuring energy consumption. Such applications may consist of many different phases and may not perform at the maximum potential performance of the available hardware, or the available hardware may project a bottleneck for the performance. 
...
drastically different enery traces we have limited our discussion to running applications on different GPUs (A100, V100) and different epochs (2, 30, 70). These traces are depicted in in Figure
\ref{fig:energy-graphs} A-F.  To better showcase the impact of the different phases of the application we have augmented the traces with different colors for the different phases of the application.
We distinguish the phases called {\em Initialize},  {\em Training}, {\em Best Fit Prediction}, {\em Visualize}, and {\em Final plots}. The meaning of these phases have been explained in Section \ref{sec:perf-runtime}. And used different colors in the Figure \ref{fig:energy-graphs} to distinguish them better.

Important to note is that for the training  phase (colored in green) the energy values of the GPU are significantly higher then in the rest of the application indicating a much higher utilization of the GPU during that phase. 
...
abscissa is significantly larger for the V100. Hence overall Energy consumption per second will be significantly larger. The fluctuations of the energy in the training phase can be explained through the various repeated processes that exists within training DL application.

\TODO{All}{Figure 9 A, B, C needs in the graphs to have labels A100, V100 P100, K80, e.g. capitol letter}

To compare the energy values between different GPUs such as A100, V100, P100 we can calculate the {\em Energy Trace Consumption} defined as the sum of all Energy values in an energy trace 
...
Hence the energy per epoch for lower numbers of epochs will be small, while for larger numbers the energy per epoch will be higher. However, the will not change much with higher numbers of epochs as demonstrated in Figure~\ref{fig:energy-graphs-compare} A.
We also see that the K80 uses a significant higher energy then the other GPUs, with the A100 performing best. For K80 and 70 epochs we could not obtain a value as it was outside the allowed time to run applications on the supercomputer defined by the centers policy.
We also see that the standard deviation for the kWh/Epoch value becomes smaller with increasing number of epochs. 
...
In Figure~\ref{fig:energy-graphs-compare} B, we also depicted the Energy trace per epoch. However in contarst to Figure~\ref{fig:energy-graphs-compare} A that were created using the data on an NFS storage devce, we included here the values obtained for data hosted on a localscratch device using NVMe storage. 
Here we see a similar average energy use per epoch. 

\TODO{JP and Knuuti}{Figure~\ref{fig:energy-graphs-compare} B should add four bars, a100 NFS and localscrath, v100 NFS and localscratch}

This comparision of using localscarcth vs. NFS mounted data shows that 

\TODO{JP and KNuuti}{TBD}


Hence it offers a clear insight how a filesystem can negatively impact the energy as the runtime is enhenced and the GPU time to cool down or adjust for reducing the energy is to small to have an impact.
...
This graph is important, as it shows the impacts of the energy usage of the GPU throughout the runtime and when compared to the plots in Figure~\ref{fig:energy-graphs-compare} A and B,  outlier events, such as a potential interaction from other users sharing the benchmark infrastructure can be identified.
}
...
\begin{figure}[htb]
  ...
  \begin{center}
       ...
       {\bf (A)} Average energy usage per epoch, including only to the Training event energy traces
        ...
        \includegraphics[height=0.29\textheight]{images/total_kWh_per_epoch_new.pdf}

         {\bf (B)} Average energy usage per epoch, for all energy traces for all timer events.

        \includegraphics[height=0.28\textheight]{images/total_epoch_vs_watts_new.pdf}
