...
efforts culmulating in the need for benchmark carpentry for scientists
using advanced computational resources. The paper also presents
several benchmarks while focussing on the accuracy of the
...
on Google Collab.  Due to the integration of multiple applications
...
\begin{figure}[htb]
  \centering\includegraphics[width=0.7\columnwidth]{images/fastapi-service-highres.png}
  ...
  \bigskip
    ...
    \centering\includegraphics[width=0.70\columnwidth]{images/cc-1.png}
...
After the gode augmentations augmentation it was possible to identify
a projected runtime on various systems while running at predifined
times. As our ML code performs about linearly with incresing epochs it
was first important to predict the performance as to create accurate
reservation requests on the HPC system.

Figure~\ref{fig:performance-projection} A shows the time measured for
a 2 epoch case while the botom portion of the graph indicates the time
spend in the GPU and the top for preparation and data postprocessing.
We repeated such experiments for small er numbers of epochs in order
to project the time needed. This allowed us to estimate the
anticipated runtime for the experiments when varying the number of
epchs between 2 and 70. The total runtime for the initial
configurations using the HPC recommended file system. Is showcased in
Figure~\ref{fig:performance-projection} B. In addtion we adde the
times opbtained on a desktop using an AMD 5950X with an RTX30990 and
128 GB of memory while all data was located on an NVMe. The data on
the HPC system was located as recommended by the cebter on an NSF
storage system.  As we see the desktop with the RTX3090 GPU performed
significantly faster then the HPC compute node equipped with any other
GPU including an A100.  To showcase the different runtimes we showcase
the results for a 2 epoch case in Table \ref{tab:2-epoch-case}. To our
initial surprise we found that the RTX3090 machine was originally
almost 2.67 times better than the HPC machine with an A100.

This desktop also significantly outperformed the application when it
was executed on google collab. In face the time on Google collab
althoough good for small epochs quickly deterioted with increasing
number of epoch due to the older GPUs available at that time on Google
Collab.
...
machine. Future expansions of the HPC machine include 28TB of of fast
...
training at about the epoch value of 30-33 (see
Figure~\ref{fig:loss}).  However, we have constantly also run higher
numbers of epochs just to be sure and found our best accuracy values
when using 70 epochs. We heve not undertaken studies with more epochs
and hope to conduct them in future.
...
\TODO{Verify if the ranges seek backwards or forwards when sampling

  Where does this go to:
  
  An epoch value of 30 and the hyperparameter {\em Now 2wk+26AV} which
  has an NNSE value for the training of 0.5218 and a validation value of
  0.502300

}

For the rest of the experiments we have chosen different for input and
output vectors that are used to generate the accuracy using our deep
...
\begin{itemize}
  ...
  \item The term {\em Now} means the most recent date from the dataset,
...
example, a value of {\em 1Y 2wk+26AVG} is the resulting dataset
starting 1 year, and including 26 observations from this point.  As
another example, the value of {\em 1 Year Back} is a single 2 week
occurrence one year ago from the rolling training date.


\section{Competitive Accuracy Benchmarks}

In this section we summarize the competitive accuracy nemchmark where
we use the nomencalture for the input and output hyperparameters
defining the time period used for the training and validation.  To
simplify our presentation we have chosen for this papers experiments
using 2, 30, and 70 epochs. We took all results and sorted them for
training and validation seperatly by the accuracy values. This leads
to a ranking of the experiments by accuracy. In Table
\ref{tab:ranking-accuracy} we present the top 20 acuracy values, while
in Figure ??? we created a histogram showcasing how many accuracy
values fall into a certain bucket over all experiments.
...
value it is also of advantage to unalyze which parameter sets lead in
general to the best results. For this we are depicting in Figure
\ref{fig:NNSE-comparison-a100}. Here we compare the histogram of all
NNSE values for training and validation
...
compute centers due to the high cost associated with it. This not only
includees common measurements such as PUE but also the measurement of
application oriented energy consumption to build for example a ranking
as projected in the Green500 list \citep{green500}.

Obviously it is of interrest to also be able to project energy
consumption of algorithmes so energy resource restrictions and cost
can be analyzed. We have however also identified an important aspect
of providing energy traces that we discuss next.

While observing initially the fast runtime of the RTX3090 on a custom
...
Here we augmented the traces also with different phases. WHiel the
green area indicates the use of the GPU shows that the energy values
recorded are increased over that period. Other phases dealing with
data reparation only show a modest use of the GPU. Furthermore we have
meausred the energy consumption between different GPUS and in Figure
\ref{fig:energy-graphs} we showcase the differentiation between A100
and V100.
...
This paper summarizes an enormous body of work that adresses multiple
aspects in machine learning benchmarking. We identified that
benchmarking can lead to a significant educational contribution to
students and researchears. We identified that not only software
...
comput coordination and workflow management system in two components
called cloudmesh-sbatch and cloudmesh-cc that allow benchmark jpbs to
be autpmatically generated from hyperparameter permutations. It also
allows us to coordinate such benchmarks on different machines. We have
identified the patterns of scelections, cooperation and competition
which are part of a benchmark workflows. Furthermore we have improved
...
mllog library osed by MLCommons application is simpler and is huan
readable. Lastly, our benchmarks had significant impact on the
operation and accassibility of software on our educationl cluster we
used. Plans for new filesystem management and updated compute nodes
exite us to conduct more such studies. We intend to submit our results
to the MLCommons Science Working groups as the earthquake forecasting is one
of their benchmark codes and efforts.
