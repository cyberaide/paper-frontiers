...
\begin{figure}[htb]
    \centering\includegraphics[width=0.6\columnwidth]{images/TFT.pdf}
...
\begin{itemize}
  \item {\bf Static Known Inputs (5):} 4 space-filling curve labels of fault grouping, linear label of pixel
...
For further information on the current TFT model, review~\citep{fox2022-jm}.

\TODO{Gregor}{We are over figure limit by +1. Let me know if I should remove my TFT Model Architecture figure. -Thomas}

\TODO{Butler}{The TFT model is not sufficiently explained}
...
\TODO{All}{figure 4 seems now relay wrong change values back, but green value is slower then red value, should that be the other way around. BUt even then its to slow to be localscratch, so what is measured here ?????. I think originally the green line was V100. Can we get new timer values for A100 and V100?}
...
\TODO{Jake}{explain Figure \ref{fig:energy-graphs-compare}}
